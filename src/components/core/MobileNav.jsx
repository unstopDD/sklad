...
const MobileNavItem = ({ to, icon: Icon, label, end = false, onClick }) => (
    <NavLink
        ...
        onClick={onClick}
        className={({ isActive }) =>
            `flex flex-col items-center justify-center gap-1 flex-1 py-2 transition-colors ${isActive ? 'text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400'
            }`
        }
    >
        <Icon size={20} />
...
const MobileNav = ({ onMenuClick }) => {
    ...
    return (
        <nav className="fixed bottom-0 left-0 right-0 z-50 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800 flex items-center justify-around md:hidden pb-safe">
            ...
            <MobileNavItem to="/app/products" icon={Package} label={t.nav.productsBrief || 'Товари'} />
            <MobileNavItem to="/app/production" icon={Factory} label={t.nav.productionBrief || 'Цех'} />
            <button
                onClick={onMenuClick}
