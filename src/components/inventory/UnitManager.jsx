...
const UnitManager = () => {
    ...
    const handleSubmit = (e) => {
        ...
        if (!newUnit.trim()) {
            setError(t.units.errorEmpty);
            return;
        }

        if (units.includes(newUnit.trim())) {
            setError(t.units.errorExists);
            return;
        }

        addUnit(newUnit.trim());
        addToast(t.units.toastAdded.replace('{unit}', newUnit.trim()), 'success');
        setNewUnit('');
        setError('');
    };
    ...
    const handleRemove = (unit) => {
        removeUnit(unit);
        addToast(t.units.toastDeleted.replace('{unit}', unit), 'success');
    };
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="space-y-4">
                <h3 className="text-lg font-medium text-[var(--text-main)]">{t.units.availableTitle} ({units.length})</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3">
                    {units.map((unit) => (
                        ...
                        >
                            <span className="font-mono font-medium text-lg text-[var(--text-main)]">{t.unitNames?.[unit] || unit}</span>
                            <button
                                ...
                                title={`${t.common?.delete || 'Удалить'} ${unit}`}
                                aria-label={`${t.common?.delete || 'Удалить'} ${unit}`}
                            >
                                <X size={18} />
                            </button>
                        </div>
                ...
                {units.length === 0 && (
                    <div className="text-center py-12 bg-[var(--bg-card)] border border-dashed border-[var(--border)] rounded-[var(--radius)]">
                        <div className="flex flex-col items-center text-[var(--text-light)]">
                            ...
                            <p className="font-medium text-[var(--text-secondary)]">{t.units.empty}</p>
                            <p className="text-sm">{t.units.emptyDesc}</p>
                        </div>
                    </div>
                )}
            </div>
